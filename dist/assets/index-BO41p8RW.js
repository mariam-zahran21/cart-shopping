(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))a(r);new MutationObserver(r=>{for(const s of r)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&a(i)}).observe(document,{childList:!0,subtree:!0});function n(r){const s={};return r.integrity&&(s.integrity=r.integrity),r.referrerPolicy&&(s.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?s.credentials="include":r.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function a(r){if(r.ep)return;r.ep=!0;const s=n(r);fetch(r.href,s)}})();const v="shopping-cart-items";let o=L();const m=new Set;function L(){const e=localStorage.getItem(v);if(e==null)return[];try{const t=JSON.parse(e);return Array.isArray(t)?t.filter(q):[]}catch{return[]}}function q(e){if(typeof e!="object"||e===null)return!1;const t=e;return typeof t.id=="number"&&Number.isInteger(t.id)&&typeof t.quantity=="number"&&Number.isInteger(t.quantity)&&t.quantity>0}function b(){localStorage.setItem(v,JSON.stringify(o)),m.forEach(e=>e())}function w(){return o.map(e=>({...e}))}function f(){return o.reduce((e,t)=>e+t.quantity,0)}function I(e){const t=o.find(n=>n.id===e);t==null?o=[...o,{id:e,quantity:1}]:t.quantity+=1,b()}function C(e){o=o.filter(t=>t.id!==e),b()}function y(e,t){if(t<=0){C(e);return}const n=o.find(a=>a.id===e);n!=null&&(n.quantity=Math.floor(t),b())}function E(e){return m.add(e),()=>m.delete(e)}function k(){const e=document.createElement("nav");return e.className="border-b border-stone-200 bg-white",e.innerHTML=`
  ...
  `,E(()=>{const t=e.querySelector("[data-cart-count]");t!=null&&(t.textContent=String(f()))}),e}function j(){const e=document.createElement("section");return e.className="py-8",e.innerHTML=`
  ...
  `,e}function O(){const e=document.createElement("section");return e.className="py-16 text-center",e.innerHTML=`
  ...
  `,t.querySelector('[data-action="add"]')?.addEventListener("click",()=>{I(e.id)}),t}function D(e){const t=document.createElement("div"),n=x.find(a=>a.id===e.id);return n==null||(t.className="grid grid-cols-[52px_1fr_auto] items-center gap-3 border-b border-stone-200 py-3 last:border-0",t.innerHTML=`
    ...
    >&times;</button></div>
  `,t.querySelector('[data-action="remove"]')?.addEventListener("click",()=>C(n.id)),t.querySelector('[data-action="decrease"]')?.addEventListener("click",()=>y(n.id,e.quantity-1)),t.querySelector('[data-action="increase"]')?.addEventListener("click",()=>y(n.id,e.quantity+1))),t}function M(){const e=document.createElement("aside");e.className="border border-stone-200 bg-[#faf9f6] p-4";const t=w();if(e.innerHTML='<h2 class="mb-2 text-2xl font-semibold tracking-tight text-stone-800">Shopping Cart</h2>',t.length===0)return e.insertAdjacentHTML("beforeend",'<p class="py-5 text-xs text-stone-500">Your cart is empty.</p>'),e;const n=document.createElement("div");return n.className="space-y-3",t.forEach(a=>n.appendChild(D(a))),e.appendChild(n),e}function A(){const e=document.createElement("div"),t=document.createElement("h1");t.className="mb-4 text-[10px] uppercase tracking-[0.25em] text-stone-400",t.textContent="DIOR / CART",e.appendChild(t);const n=document.createElement("div");n.className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_240px]",n.appendChild(M());const a=document.createElement("aside"),r=w().reduce((u,h)=>{const N=x.find(S=>S.id===h.id);return u+(N?.price??0)*h.quantity},0);a.className="border border-stone-200 bg-[#faf9f6] p-4",a.innerHTML=`<h2 class="mb-4 text-xs font-semibold uppercase tracking-wider text-stone-800">Order summary</h2><div class="space-y-2 text-xs text-stone-500"><div class="flex justify-between"><span>Items</span><span>${f()}</span></div><div class="flex justify-between"><span>Shipping</span><span>Free</span></div><div class="mt-3 flex justify-between border-t border-stone-200 pt-3 font-semibold text-stone-800"><span>Total</span><span>${c(r)}</span></div></div><button class="mt-5 w-full bg-stone-800 px-3 py-2 text-[10px] uppercase tracking-widest text-white hover:bg-stone-700">Checkout</button>`,n.appendChild(a),e.appendChild(n);const s=document.createElement("h2");s.className="mt-7 mb-3 text-xs font-semibold uppercase tracking-widest text-stone-800",s.textContent="You may also like",e.appendChild(s);const i=document.createElement("div");i.className="grid grid-cols-2 gap-3 sm:grid-cols-4",x.slice(0,4).forEach(u=>i.appendChild(T(u))),e.appendChild(i);const d=document.createElement("section");return d.className="mt-8 border border-stone-200 bg-[#faf9f6] p-5",d.innerHTML='<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-2xl font-semibold text-stone-800">Subscribe</h2><p class="mt-1 text-xs text-stone-500">Get updates on new arrivals and offers.</p></div><form class="flex"><input type="email" required placeholder="Your email address" class="w-full border border-stone-300 bg-transparent px-3 py-2 text-xs outline-none sm:w-56" /><button class="bg-stone-800 px-4 py-2 text-[10px] uppercase tracking-widest text-white">Join</button></form></div><div class="mt-6 border-t border-stone-200 pt-4 text-[10px] font-bold uppercase tracking-widest text-black">DIOR</div>',e.appendChild(d),e}function p(e){const t=window.location.hash.slice(1)||"/store";e.replaceChildren(t==="/store"?A():t==="/about"?j():O())}function H(e){window.addEventListener("hashchange",()=>p(e)),E(()=>p(e)),p(e)}const l=document.querySelector("#app");if(l==null)throw new Error("Application root was not found.");l.className="min-h-screen bg-[#e9e8e6] text-gray-900";l.appendChild(k());const g=document.createElement("main");g.className="mx-auto max-w-5xl px-4 py-4";l.appendChild(g);H(g);
